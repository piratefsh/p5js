...
export default class ParametricPatterns {
  ...
  reset() {
    ...
    this.randVar = this.p5.random(0, this.seed);
    this.x1 = this.util.generateParametricEqn(this.width/this.p5.random(2,5), 6);
    this.y1 = this.util.generateParametricEqn(this.width/this.p5.random(2,5), 6);
    this.x2 = this.util.generateParametricEqn(this.width/this.p5.random(2,5), 6);
    this.y2 = this.util.generateParametricEqn(this.height/this.p5.random(2,5), 6);
  ...
  draw() {
    ...
    for (let i = 0; i < count; i += this.spacing) {
      ...
      const points = [this.x1(t), this.y1(t), this.x2(t), this.y2(t)]
        .map(pt => pt * this.amp);
      this.p5.line(...points);
